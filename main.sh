...
while [[ true ]]; do
  wget -q "https://mtg-vod-http.babahhcdn.com/mtg/_definst_/smil:tv3/prooviabielu/season1/20388082/20388082e_HD.smil/media_b1096000_sleng_${counter}.ts"
  if [ $? -ne 0 ];then
    break
  else
...
timestamp=$(date +%s)
cat *.ts > all.ts
ffmpeg -i all.ts -acodec copy -vcodec copy ${timestamp}.mp4
